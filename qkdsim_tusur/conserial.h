...
private:
    /// @brief Структура версии прошивки АП (X.Y.Z)
    /// @param major - X
    /// @param minor - Y
    /// @param micro - Z
    ...
    /// @brief Структура версии протокола (X.Y)
    /// @param version - X
    /// @param subversion - Y
    ...
    versionProtocol versionProtocol = {1, 2};

    /// @brief Текущие параметры со стенда
    /// @param premissions - Уровень доступа
    /// @param laserState_ - Вкл./Выкл. лазер
    /// @param laserPower_ - Мощность лазера
    /// @param signalLevels_ - Мощность на фотодетекторах
    /// @param laserPower_ - Мощность лазера
    /// @param curAngles_ - Текущие углы поворота пластин
    /// @param lightNoises - Текущяя засветка на фотодетекторах
    /// @param startLightNoises_ - Начальная засветка на фотодетекторах
    /// @param startPlatesAngles_ - Начальные углы поворота пластин
    /// @param maxSignalLevels_ - Максимальный уровень сигнала
    /// @param timeoutTime_ - Время ожидания ответа от АП (в сек.)
    /// @param rotateStep_ - Один шаг двигателя (в углах)
    /// @param maxLaserPower_ - Максимальная мощность лазера
    /// @param  maxPayloadSize - Максимальная длинна параметров в байтах
    struct StandOptions{
        adc_t premissions = 0;
        adc_t laserState_ = 0;
        adc_t laserPower_ = 0;
